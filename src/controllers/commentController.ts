...
import { getAnalysisModel, cleanJsonResponse } from "../utils/gemini";
...
export const commentOnIssue: RequestHandler = async (req, res) => {
  ...
  try {
    // 1. Post the Welcome/Staking Comment
    ...
    // 2. Generate and Post AI Summary (if title/desc provided)
    if (title && process.env.GEMINI_API_KEY) {
      console.log("🤖 Generating AI summary for issue...");
      // Run in background (don't await to block response, OR await if we want to confirm success)
      // We'll await to ensure reliability for now.
      await generateAndPostIssueSummary(
        owner,
        repo,
        issueNumber,
        title,
...
async function generateAndPostIssueSummary(
  owner: string,
  repo: string,
  issueNumber: number,
  title: string,
  ...
  try {
    const model = getAnalysisModel(
      "You are a technical product manager and software architect. Summarize the issue clearly.",
    );

    const prompt = `Please analyze this GitHub Issue and provide a summary.
     
     **Title**: ${title}
     **Description**: ${description}
     
     Generate a response in this Markdown format:
     
     ## 🤖 AI Issue Summary
     
     ### 📋 Goal
     <One sentence summary of what is requested>
     ...
     <Brief technical suggestion if applicable, else 'No specific implementation details provided'>
     
     *Generated by PullQuestAI*
     `;

    const result = await model.generateContent(prompt);
    const summary = result.response.text();

    await postIssueComment(owner, repo, issueNumber, summary);
    console.log(`✅ Posted AI Issue Summary for #${issueNumber}`);
  } catch (error) {
...
export const commentOnPrs: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      console.log(
        `❌ Contributor not found: ${author} with role 'contributor'`,
      );
    }
  } catch (error) {
  ...
  if (user && stakeAmt > 0) {
    ...
    if (userCoins >= stakeAmt) {
      console.log(
        `✅ ${author} has enough coins (${userCoins}) for stake (${stakeAmt})`,
      );
      ...
      if (linkedIssueNumber) {
        try {
          console.log(
            `🔍 Fetching and ingesting issue #${linkedIssueNumber}...`,
          );

          // Fetch complete issue data from GitHub API
          const fullIssueData = await fetchCompleteIssueData(
            owner,
            repo,
            linkedIssueNumber,
          );
...
• 🏆 **Current rank:** ${user.rank}`;
      ...
      console.log(
        `❌ ${author} doesn't have enough coins (${userCoins}) for stake (${stakeAmt})`,
      );
...
export const AddbonusXp: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      ...
      const comment = await postPRFormComment(
        owner,
        repo,
        prNumber,
        commentBody,
